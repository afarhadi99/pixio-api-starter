...
import { createSubscriptionForNative } from '@pixio/billing';

/** Create an incomplete subscription + PaymentSheet payload for native confirm. */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const payload = await createSubscriptionForNative(stripe, {
      customerId,
      priceId,
      userId: user.id,
